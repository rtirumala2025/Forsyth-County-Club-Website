...
const migrateSchools = async () => {
  ...
  const schoolData = Array.from(schools).map(schoolName => ({
    ...
    website: '',
    isActive: true,
    clubCount: allClubData.filter((club: any) => club.school === schoolName).length,
...
const migrateCategories = async () => {
  ...
  const categoryData = Array.from(categories).map((categoryName, index) => ({
    ...
    description: `Clubs related to ${(categoryName as string).toLowerCase()}`,
    icon: getCategoryIcon(categoryName as string),
    color: getCategoryColor(categoryName as string),
    isActive: true,
    clubCount: allClubData.filter((club: any) => club.category === categoryName).length,
    sortOrder: index,
    createdAt: new Date(),
    updatedAt: new Date()
  }));
