...
export default function Chatbot({
  ...
  // Config
  useEffect(() => {
    // Define ga on window if not exists for TS
  ...
  const suggestions = useMemo(() => {
    ...
    const lastBotMessage = messages.filter(m => m.type === 'bot').pop() as any;
  ...
  const pushMessage = (type, text, extra = {}) => {
    setMessages((prev) => [
      ...prev,
      {
        ...
        text,
        ts: new Date().toISOString(),
        clubs: (extra as any).clubs || [],
        suggestions: (extra as any).suggestions || [],
      },
    ]);
  };
