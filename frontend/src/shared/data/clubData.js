// Central club data file - shared across frontend and backend
const allClubData = [
  {
...
// Helper function to get clubs by school
const getClubsBySchool = (schoolName) => {
  const schoolData = allClubData.find(school => school.school === schoolName);
  if (!schoolData) return [];
  return schoolData.clubs || schoolData.club || [];
};

// Helper function to get all available schools
const getAvailableSchools = () => {
  return allClubData.map(school => school.school);
};

// Export for ES6 (frontend) and CommonJS (backend)
export { allClubData, getClubsBySchool, getAvailableSchools };

// Also export for CommonJS (backend compatibility)
