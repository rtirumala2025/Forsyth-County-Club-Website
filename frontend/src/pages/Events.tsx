...
const Events = () => {
  ...
  const filteredClubsData = useMemo(() => {
    ...
    return supabaseRecords
      .filter(r => (r.school as any)?.name === selectedSchool)
      .map(r => ({
        id: r.id,
        name: (r.club as any)?.name ?? '',
        category: (r.club as any)?.category ?? 'Other',
        sponsor: r.sponsor_name ?? '',
      }));
  }, [supabaseRecords, selectedSchool]);
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div className="space-y-3">
                  <button
                    ...
                    className={`w-full flex items-center px-3 py-2 rounded-lg transition-colors ${viewMode === 'grid' ? 'bg-blue-100 text-blue-700 border border-blue-200' : 'bg-gray-50 text-gray-700 hover:bg-gray-100'
                      }`}
                  >
                  ...
                  <button
                    ...
                    className={`w-full flex items-center px-3 py-2 rounded-lg transition-colors ${viewMode === 'calendar' ? 'bg-blue-100 text-blue-700 border border-blue-200' : 'bg-gray-50 text-gray-700 hover:bg-gray-100'
                      }`}
                  >
