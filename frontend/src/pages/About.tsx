...
const About = () => {
  ...
  const { data: supabaseRecords, isLoading } = useSupabaseClubs();

  // Transform Supabase data into the grouped shape
  ...
  const totalClubs = allClubData.reduce((total, school) =>
    total + school.clubs.length, 0
  );
  const totalCategories = new Set(allClubData.flatMap(school =>
    school.clubs.map((club: any) => club.category)
  )).size;
  const stats = [
    {
      icon: Building,
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {availableSchools.map((schoolName, index) => {
              ...
              const clubCount = schoolData?.clubs?.length || 0;
              return (
                <button
                  key={index}
                  onClick={() => navigate('/')}
