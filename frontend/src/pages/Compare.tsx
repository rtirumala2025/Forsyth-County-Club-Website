...
const Compare = () => {
  ...
  const { data: supabaseRecords, isLoading, isError } = useSupabaseClubs();
  ...
  const allClubData = useMemo(() => {
    ...
    for (const rec of supabaseRecords) {
      ...
      grouped.get(schoolName)!.push({
        ...
        meetingFrequency: rec.meeting_details ?? '',
      });
    }
  ...
  const availableSchools = useMemo(
    () => allClubData.map(s => s.school),
    [allClubData]
  );

  // Filter clubs by selected school
  const filteredClubsData = useMemo(() => {
    const schoolData = allClubData.find(s => s.school === selectedSchool);
                  ...
                  <div className="absolute top-full left-0 mt-2 w-80 bg-white border border-gray-300 rounded-lg shadow-xl z-50 max-h-60 overflow-y-auto">
                    {availableSchools.map(school => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-2 hover:bg-gray-50 transition-colors ${selectedSchool === school ? 'bg-blue-50 text-blue-800' : ''
                          }`}
                      >
                        {school}
                      </button>
